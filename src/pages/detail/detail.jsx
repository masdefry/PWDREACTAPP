...
export default function DetailProduct(){
    ...
    let onGetData = async() => {
        try {
            let response = await axios.get(`https://my-json-server.typicode.com/masdefry/pwdjsonserver/products/${productId.id}`)
    ...
    let onAddOrder = async() => {
        try {
            ...
            let checkExist = await axios.get(`https://my-json-server.typicode.com/masdefry/pwdjsonserver/cart?idProduct=${data.id}`)

            if(checkExist.data.length === 0){
                let response = await axios.post('https://my-json-server.typicode.com/masdefry/pwdjsonserver/cart', dataToSend)
            }else{
                let newQuantity = checkExist.data[0].quantity + 1
                let update = await axios.patch(`https://my-json-server.typicode.com/masdefry/pwdjsonserver/cart/${checkExist.data[0].id}`, {quantity: newQuantity})
                console.log(update)
            }   
        } catch (error) {
