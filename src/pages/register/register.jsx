...
export default function Register(props){
    ...
    let onSubmit = async() => {
        try {
            ...
            let checkEmail = await axios.get(`http://https://my-json-server.typicode.com/masdefry/pwdjsonserver/users?email=${inputEmail}`)
            let checkUsername = await axios.get(`http://https://my-json-server.typicode.com/masdefry/pwdjsonserver/users?username=${inputUsername}`)
            
            if(checkEmail.data.length === 0 && checkUsername.data.length === 0){
                // Post
                await axios.post('http://https://my-json-server.typicode.com/masdefry/pwdjsonserver/users', {username: inputUsername, email: inputEmail, password: inputPassword})
